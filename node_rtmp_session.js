    ...
    *
    handleData() {
        ...
        while (this.isStarting) {
            ...
            let chunkBody = yield chunkBodySize
            message.receivedLength += chunkBodySize;
            if (chunkBody) message.chunks.push(chunkBody);
            if (message.receivedLength == message.messageLength) {
                ...
                if (message.chunks.length) {
                    this.handleRTMPMessage(message, Buffer.concat(message.chunks));
                    message.receivedLength = 0;
    ...
    createRtmpMessage(rtmpHeader, rtmpBody) {
        ...
        let rtmpBodyPos = 0;
        let chunkBodys = [];
        let type3Header = Buffer.from([(3 << 6) | rtmpHeader.chunkStreamID]);
        ...
        let headerBufs = Buffer.from([(formatTypeID << 6) | rtmpHeader.chunkStreamID, timestamp[0], timestamp[1], timestamp[2], (rtmpBodySize >> 16) & 0xff, (rtmpBodySize >> 8) & 0xff, rtmpBodySize & 0xff, rtmpHeader.messageTypeID, rtmpHeader.messageStreamID & 0xff, (rtmpHeader.messageStreamID >>> 8) & 0xff, (rtmpHeader.messageStreamID >>> 16) & 0xff, (rtmpHeader.messageStreamID >>> 24) & 0xff]);
        if (useExtendedTimestamp) {
            let extendedTimestamp = Buffer.from([(rtmpHeader.timestamp >> 24) & 0xff, (rtmpHeader.timestamp >> 16) & 0xff, (rtmpHeader.timestamp >> 8) & 0xff, rtmpHeader.timestamp & 0xff]);
        ...
        switch (dataMessage.cmd) {
            case '@setDataFrame':
                if (dataMessage.dataObj != null) {
                    ...
                    this.metaData = this.amfWriter.encodeAmf0Data(opt).clone();
                }
                break;
            default:
                break;
    ...
    sendRtmpSampleAccess() {
        ...
        let rtmpBody = this.amfWriter.encodeAmf0Data(opt).valid();
        let rtmpMessage = this.createRtmpMessage(rtmpHeader, rtmpBody);
    ...
    sendStatusMessage(id, level, code, description) {
        ...
        let opt = {
            ...
            transId: 0,
            cmdObj: null,
            info: { level, code, description }
        };
        let rtmpBody = this.amfWriter.encodeAmf0Cmd(opt).valid();
        let rtmpMessage = this.createRtmpMessage(rtmpHeader, rtmpBody);
        this.socket.write(rtmpMessage);
    }
    ...
    pingRequest() {
        ...
        let rtmpBody = Buffer.from([0, 6, (currentTimestamp >> 24) & 0xff, (currentTimestamp >> 16) & 0xff, (currentTimestamp >> 8) & 0xff, currentTimestamp & 0xff])
        let rtmpMessage = this.createRtmpMessage(rtmpHeader, rtmpBody);
        this.socket.write(rtmpMessage);
